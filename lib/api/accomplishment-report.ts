...
export type GenerateAccomplishmentReportPayload = {
  templateId: string;
  firstName?: string;
  lastName?: string;
  designation?: string;
...
export type GenerateAccomplishmentReportResult = {
  blob: Blob;
  fileName: string;
};
...
export const generateAccomplishmentReport = async (
  ...
  const response = await fetch("/api/v1/generate-accomplishment-report", {
    method: "POST",
    credentials: "include",
    body: formData,
  });
