import type { AccomplishmentReportPeriod } from "./accomplishment-report-period";
...
export const getStartDay = (period: AccomplishmentReportPeriod): number =>
  period.half === 1 ? 1 : 16;

export const getEndDay = (period: AccomplishmentReportPeriod): number => {
  const lastDay = getDaysInMonth(period.year, period.month);
  return period.half === 1 ? 15 : Math.min(lastDay, 31);
};

export const formatPeriodLabel = (period: AccomplishmentReportPeriod): string => {
...
export const getSheetNameForPeriod = (period: AccomplishmentReportPeriod): string => {
  const monthName = MONTH_NAMES[period.month - 1];
  const start = getStartDay(period);
  const end = getEndDay(period);
