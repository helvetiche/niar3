...
export const SECURITY_RESPONSE_HEADERS: Record<string, string> = {
  ...
  // Next.js App Router emits inline bootstrap/hydration scripts; without 'unsafe-inline'
  // (or a full nonce pipeline), the browser blocks them and the client bundle never runs.
  // Prefer nonce-based script-src in the future; see docs/SECURITY.md.
  "Content-Security-Policy":
    "default-src 'self'; " +
