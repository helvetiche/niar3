...
export async function proxy(request: NextRequest) {
  ...
  const isAuthRoute =
    pathname.startsWith('/api/v1/auth/') || pathname.match(/^\/(login|register|forgot-password)/)

  // Distributed rate limiting (Upstash Redis)
  if (isRateLimitEnabled()) {
    const identifier = getClientIdentifier(request)
    const limiter = isAuthRoute ? authRateLimit : isApiRoute ? apiRateLimit : publicRateLimit
