...
interface Issue {
  lotCode: string;
  issueType: 'missing_in_consolidated' | 'missing_in_ifr' | 'principal_mismatch' | 'penalty_mismatch' | 'area_mismatch' | 'old_account_mismatch' | 'total_mismatch';
  field: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!consolidatedFile) {
      return NextResponse.json({ error: 'No consolidated file provided' }, { status: 400 });
    }
    ...
    const consolidatedWorkbook = XLSX.read(consolidatedBuffer, { type: 'buffer' });
    ...
    for (const [lotCode, consolidated] of consolidatedData) {
    ...
    const summary = {
      ...
      matchingLots: expectedData.size - issues.filter(i => i.issueType === 'missing_in_consolidated').length,
